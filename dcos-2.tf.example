...
# Bootstrap
module "dcos-bootstrap-instance-2" {
...
# Master
module "dcos-master-instances-2" {
...
# Private Agent
module "dcos-privateagent-instances-2" {
...
# Public Agent
module "dcos-publicagent-instances-2" {
...
module "dcos-elb-2" {
  ...
  master_instances                 = ["${module.dcos-master-instances-2.instances}"]
  public_agent_instances           = ["${module.dcos-publicagent-instances-2.instances}"]
}

##############################################
# Create the Execution for the DC/OS Cluster
##############################################
module "dcos-install-2" {
  source               = "dcos-terraform/dcos-install-remote-exec/null"
  bootstrap_ip         = "${module.dcos-bootstrap-instance-2.public_ip}"
  bootstrap_private_ip = "${module.dcos-bootstrap-instance-2.private_ip}"
  bootstrap_os_user    = "${module.dcos-bootstrap-instance-2.os_user}"

  master_ips         = ["${module.dcos-master-instances-2.public_ips}"]
  master_private_ips = ["${module.dcos-master-instances-2.private_ips}"]
  masters_os_user    = "${module.dcos-master-instances-2.os_user}"
  num_masters        = "${var.num_masters}"

  private_agent_ips      = ["${module.dcos-privateagent-instances-2.public_ips}"]
  private_agents_os_user = "${module.dcos-privateagent-instances-2.os_user}"
  num_private_agents     = "${var.num_private_agents}"

  public_agent_ips      = ["${module.dcos-publicagent-instances-2.public_ips}"]
...
# Locals Vars
locals {
  bootstrap_ips-2      = "${join("\n", flatten(list(module.dcos-bootstrap-instance-2.public_ip)))}"
...
[bootstrap]
${local.bootstrap_ips-2}

[masters]
${local.masters_ips-2}

[private-agents]
${local.private_agents_ips-2}

[public-agents]
${local.public_agents_ips-2}

[public-agent-lb]
${local.public-agent-lb-2}
